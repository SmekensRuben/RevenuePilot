...
export default function AddSupplierForm({ onAdd }) {
  ...
  const handleSubmit = async e => {
    ...
    setSaving(true);
    try {
      // Key aanmaken (lowercase, underscores, geen spaties)
      const key = form.name.trim().toLowerCase().replace(/[^a-z0-9]/gi, "_");
  ...
  return (
    ...
    >
      <h3 className="font-bold text-lg mb-2 text-marriott">Nieuwe leverancier toevoegen</h3>
      ...
      <div className="flex flex-col gap-1">
        <label className="text-sm font-medium text-gray-700">Klantnummer</label>
        <input
          className="border rounded px-3 py-2 outline-marriott focus:ring-2 focus:ring-marriott/30"
          placeholder="(optioneel)"
          value={form.customerNr}
          onChange={e => setForm(f => ({ ...f, customerNr: e.target.value }))}
        />
      </div>
      <div className="flex flex-col gap-1">
        ...
        <input
          className="border rounded px-3 py-2 outline-marriott focus:ring-2 focus:ring-marriott/30"
          placeholder="Leverancier e-mail"
          type="email"
          value={form.email}
